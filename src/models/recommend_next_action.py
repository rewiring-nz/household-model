...
def recommend_next_action(household: Household) -> Recommendation:
    ...
    # 2. First EV
    if n_vehicles_to_electrify(household.vehicles) > 0:
        action = RecommendationActionEnum.VEHICLE

    # 3. Space heater
    if should_electrify(household.space_heating, electrify_space_heating):
        action = RecommendationActionEnum.SPACE_HEATING

    # 4. Water heater
    if should_electrify(household.water_heating, electrify_water_heating):
        action = RecommendationActionEnum.WATER_HEATING

    # 5. Cooktop
    if should_electrify(household.cooktop, electrify_cooktop):
        action = RecommendationActionEnum.COOKING

    # 6. Battery
    if should_install(household.battery):
        action = RecommendationActionEnum.BATTERY

    # 7. All other vehicles
    if n_vehicles_to_electrify(household.vehicles) > 1:
        action = RecommendationActionEnum.SPACE_HEATING
